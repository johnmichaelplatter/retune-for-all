        ...
        terminal.draw(|f| {
            let chunks = Layout::default()
                ...
                .constraints([
                    ...
                    Constraint::Length(10), // Guitar Grid (8 strings + 2 borders)
            ...
            let inner_grid_area = grid_block.inner(chunks[2]);
            f.render_widget(grid_block, chunks[2]);
            ...
            f.render_widget(Paragraph::new(log_text).block(Block::default().title(" Logs ").borders(Borders::ALL)), chunks[3]);

            // --- COMMAND INPUT ---
            let input_style = if ui_state.focus == Focus::CommandInput { Style::default().fg(Color::Yellow) } else { Style::default() };
            f.render_widget(Paragraph::new(format!("> {}", ui_state.input)).style(input_style).block(Block::default().title(" Command Input (Presets 1-9, '0', 'q') ").borders(Borders::ALL)), chunks[4]);
        })?;
